...
class PersistedClassSubject : ClassSubject, SequentialFileSerializable<PersistedClassSubject> {
    override var pointer: EntityPointer<PersistedClassSubject>? = null
    override val serializer: SequentialFileSerializer<PersistedClassSubject> get() = Serializer
    ...
    object Serializer: SequentialFileSerializer<PersistedClassSubject> {
        ...
        override fun toBytes(file: SequentialFile, entity: PersistedClassSubject) = dataBytes { data ->
            entity.apply {
                ...
                entity.fields.map { file.put(it) }.let { data.writeEntityPointersList(it) }
                entity.methods.map { file.put(it) }.let { data.writeEntityPointersList(it) }
                file.put(entity.readingContext, ReadingContextSerializer).let { data.writeEntityPointer(it) }
                data.writeStringsList(entity.path)
                entity.dependencies.map { file.put(it) }.let { data.writeEntityPointersList(it) }
                data.writeString(entity.sourcePath)
            }
        }

        override fun fromBytes(file: SequentialFile, bytes: ByteArray): PersistedClassSubject {
